...
function LenderBorroweDetail() {
    ...
    const reload=()=>{
        ...
        axios.get(`http://localhost:8080/requestOrderDetail/${data.row.productid}`).then((response) => {
...
useEffect(() => {
    ...
    axios.get(`http://localhost:8080/changeorderStatusAdmin/${data.row.productid}`).then((response) => {
    })
    }
    const completeOrder=()=>{
